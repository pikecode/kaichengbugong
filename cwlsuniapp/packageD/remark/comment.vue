<template>
	<view>
		...
		<view class="comment" v-for="(res, index) in commentList" :key="res.id">
			<view class="left" @click="goPage('/packageA/goods/detail?id=' + res.goods_id)"><image :src="res.goods && res.goods.image" mode="aspectFill"></image></view>
			<view class="right">
				<view class="content u-line-2" @click="goPage('/packageA/goods/detail?id=' + res.goods_id)">{{ res.goods && res.goods.title }}</view>
				<view class="reply-box">
					<view class="u-tips-color">
						评分:
						<u-rate :disabled="true" :current="res.star"></u-rate>
					</view>
					<view class="item u-m-t-10" @click="goPage('/packageA/goods/detail?id=' + res.goods_id)">
