<template>
	<view>
		...
		<u-modal v-model="modeShow" content="您还没有收货地址呢,去添加!" :show-cancel-button="true" @confirm="goPage('/packageD/address/addedit')"></u-modal>
	...
	methods: {
		...
		submit() {
			...
			this.$api.exchange(this.form).then(res => {
				...
				if (res.code == 1) {
					setTimeout(() => {
						this.goPage('/packageD/score/order');
					}, 1500);
				}
			});
		}
