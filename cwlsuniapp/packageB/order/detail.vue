	...
	<view class="u-skeleton">
		...
		<view class="order u-skeleton-rect">
			<view class="item" v-for="(item, index) in info.order_goods" :key="index">
				<view class="left" @click="goPage('/packageA/goods/detail?id=' + item.goods_id)"><image :src="item.image" mode="aspectFill"></image></view>
				<view class="content">
					<view class="title u-line-2" @click="goPage('/packageA/goods/detail?id=' + item.goods_id)" v-text="item.title"></view>
					<view class="type u-line-2" @click="goPage('/packageA/goods/detail?id=' + item.goods_id)" v-text="item.attrdata || ''"></view>
					<view class="u-flex">
						<!-- 申请售后：已发货,订单正常 -->
						<view
							class="u-m-r-10"
						...
						<view
							...
							@click="goPage('/packageD/order/apply?id=' + item.id)"
						>
						...
						<!-- 申请售后：退货中,已退货,已退款 -->
						<view class="" v-if="[1, 2, 3, 6].includes(item.salestate)" @click="goPage('/packageD/order/aftersale?id=' + item.id)">
							<u-tag text="查看售后" mode="plain" shape="circle" size="mini"></u-tag>
						</view>
					</view>
				</view>
				<view class="right" @click="goPage('/packageA/goods/detail?id=' + item.goods_id)">
					<view class="price">
						￥{{ priceInt(item.price) }}
			...
			<view class="u-flex u-flex-1 u-row-right u-m-l-10">
				...
				<view class="u-m-l-15" v-if="info.shippingstate == 2 && !info.orderstate && info.paystate == 1">
					<u-button
						size="mini"
						hover-class="none"
						@click="goPage('/packageD/remark/remark?order_sn=' + info.order_sn)"
				...
				<view class="u-m-l-15" v-if="!info.paystate && !info.orderstate">
					<u-button
						...
						shape="circle"
						:plain="true"
						@click="goPage('/packageD/order/payment?order_sn=' + info.order_sn)"
					>
						立即支付
					</u-button>
				</view>
