				...
				<view class="page-view">
					<view class="class-item">
						<view class="item-title u-flex u-row-between">
							<text>{{ row.name }}</text>
							<view class="" @click="goPage('/packageA/goods/goods?category_id=' + row.id)">
								<text class="u-tips-color">更多</text>
								<u-icon name="arrow-right-double" color="#999" size="28"></u-icon>
							</view>
						</view>
						<view class="item-container">
							<view class="thumb-box" v-for="(item1, index1) in row.goods" :key="index1" @click="goPage('/packageA/goods/detail?id=' + item1.id)">
