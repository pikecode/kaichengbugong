...
<script>
	...
	export default {
		...
		methods: {
			getItemType(item) {
				// console.log(item)
			  if (typeof item === 'string') return  this.checkMediaType(this.getSource(item)) ? 'video' : 'image'
			  if (typeof item === 'object' && this.name) {
			    if (!item.type) return this.checkMediaType(this.getSource(item)) ? 'video' : 'image'
